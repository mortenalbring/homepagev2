...
export interface WindowManagerControls {
    ...
    openPopup: (popupId: string, initialSize?: Size) => void;
...
export function useWindowManager(): WindowManagerControls {
    ...
    // Action creators
    const openPopup = useCallback((popupId: string, initialSize?: Size) => {
    ...
    const handleItemOpen = useCallback((action: OpenAction) => {
        if (action.type === 'folder') {
            ...
            dispatch({type: 'OPEN_POPUP', popupId: action.id, initialSize: action.initialSize});
        }
    }, []);
